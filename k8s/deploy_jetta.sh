...
yq eval -i "select(di == 0).spec.template.spec.containers[0].image = \"registry.digitalocean.com/protocol/libp2p/hydra-booster:${TAG}\"" k8s/jetta.yaml
kubectl apply -f jetta.yaml
